...
void doGraphics(State &gs) {
    ...
    */
    ...
    // Blocca aggiornamenti se in pausa
    if (gs.pauseTimer == 0) {

        for(int i = 0; i < gs.ghosts.size(); i++){
            gs.ghosts[i].update(gs.tileMap, gs.player);
        }

        gs.player.update(gs.tileMap, gs.score, gs.ghosts);
        gs.gameManager.update(&gs.pauseTimer);
    } else {
        gs.pauseTimer--;
    }
