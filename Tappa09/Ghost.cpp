...
void Ghost::setState(GhostState newState, bool force /* = false */, int currentLevel /* = -1 */) {
    ...
    switch (currentState) {
        ...
        case GhostState::Frightened:
            ...
            switch(currentLevel) {
                case 0:
                case 1:
                    stateTimer = 360; // 6 seconds
                    break;
                case 2:
                case 6:
                case 10:
                    stateTimer = 300; // 5 seconds
                    break;
                case 3:
                    stateTimer = 240; // 4 seconds
                    break;
                case 4:
                case 14:
                    stateTimer = 180; // 3 seconds
                    break;
                case 5:
                case 7:
                case 8:
                case 11:
                    stateTimer = 120; // 2 seconds
                    break;
                case 9:
                case 12:
                case 13:
                ...
                case 16:
                case 18:
                    stateTimer = 60; // 1 second
                    break;
                case 17:
                    stateTimer = 0; // 0 second
                    break;
                default: // currentLevel > 18
                    stateTimer = 0; // 0 second
                    break;
            }
...
void Ghost::updateVelocity(int currentLevel, GhostState state) {
    
    if (state == GhostState::Frightened) {
        if (currentLevel == 1) {
            velocity = FULL_SPEED * 0.5f;
        } else if (currentLevel >= 2 && currentLevel <= 4) {
            velocity = FULL_SPEED * 0.55f;
        } else if (currentLevel >= 5 && currentLevel <= 20) {
            velocity = FULL_SPEED * 0.6f;
        } else { // level >= 21
            velocity = FULL_SPEED * 0.6f;
        }
    } else if (state == GhostState::Dead) {
        velocity = FULL_SPEED * 1.5f; // Original dead speed is 2x but the game collision bugs out
    } else {
        if (currentLevel == 1) {
            velocity = FULL_SPEED * 0.75f;
        } else if (currentLevel >= 2 && currentLevel <= 4) {
            velocity = FULL_SPEED * 0.85f;
        } else if (currentLevel >= 5 && currentLevel <= 20) {
...
void Ghost::loadNextLevel(int currentLevel) {

    // Update the ghost velocity based on level progression
    updateVelocity(currentLevel); // Reset to default speed for the next level
...
void Ghost::update(TileMap& tileMap, Player& player, Ghost& blinky) {
    ...
    float newVelocity;
    if(isSpawning) {
        newVelocity = SPAWN_VELOCITY; // Spawn speed
    } else {
        newVelocity = velocity; // not spawning
    }
    switch (direction) {
        case 1: // Up
